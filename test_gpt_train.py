...
def test_tokenizer_build_vocab():
    tok = gpt_train.BPETokenizer(max_vocab_size=200)
...
def test_tokenizer_roundtrip():
    tok = gpt_train.BPETokenizer(max_vocab_size=500)
    tok.build_vocab(SAMPLE_TEXTS)
...
def test_tokenizer_backward_compat():
    ...
    tok = gpt_train.WordTokenizer(max_vocab_size=200)
    tok.build_vocab(SAMPLE_TEXTS)
...
def test_dataset_len():
    tok = gpt_train.BPETokenizer(max_vocab_size=500)
    tok.build_vocab(SAMPLE_TEXTS)
    ds = gpt_train.WordDataset(SAMPLE_TEXTS, tok, seq_length=10)
    assert ds.__len__() > 0
...
def test_dataset_empty():
    ...
    with tempfile.NamedTemporaryFile(suffix=".npy", delete=False) as f:
        tmp_cache = f.name
    try:
        ds = gpt_train.WordDataset(["hi"], tok, seq_length=100, cache_file=tmp_cache)
        assert len(ds) == 0
    finally:
...
def test_dataset_eos_tokens():
    ...
    ds = gpt_train.WordDataset(SAMPLE_TEXTS[:2], tok, seq_length=5)
...
def test_attention_init():
    ...
    attn = gpt_train.CausalSelfAttention(cfg)
...
def test_block_init():
    ...
    block = gpt_train.Block(cfg)
    assert isinstance(block.attn, gpt_train.CausalSelfAttention)
...
def test_gpt_mini_init():
    cfg = {"n_layer": 2, "n_head": 4, "head_dim": 32, "seq_length": 64}
    model = gpt_train.GPTMini(cfg, vocab_size=50)
...
def test_gpt_mini_single_layer():
    ...
    model = gpt_train.GPTMini(cfg, vocab_size=20)
...
def test_gpt_mini_large_model():
    ...
    model = gpt_train.GPTMini(cfg, vocab_size=200)
...
def test_generate_text():
    ...
    tok = gpt_train.BPETokenizer(max_vocab_size=200)
    tok.build_vocab(SAMPLE_TEXTS)
    model = gpt_train.GPTMini(cfg, tok.vocab_size)
    model.eval()
    text = gpt_train.generate_text(model, tok, "the quick brown", max_new_tokens=10, device="cpu")
    assert isinstance(text, str)
    assert len(text) > 0
...
def test_generate_text_eos():
    ...
    model = gpt_train.GPTMini(cfg, tok.vocab_size)
    model.eval()
    text = gpt_train.generate_text(model, tok, "the", max_new_tokens=50, device="cpu")
    assert isinstance(text, str)
...
def test_generate_text_temperature():
    ...
    model = gpt_train.GPTMini(cfg, tok.vocab_size)
    model.eval()
    t1 = gpt_train.generate_text(model, tok, "hello", max_new_tokens=10, temperature=0.1, device="cpu")
    t2 = gpt_train.generate_text(model, tok, "hello", max_new_tokens=10, temperature=1.0, device="cpu")
...
def test_config_hash_stable():
    cfg = SMALL_CONFIG
    h1 = gpt_train.config_hash(cfg)
    h2 = gpt_train.config_hash(cfg)
    assert h1 == h2
    assert len(h1) == 64
...
def test_config_hash_excludes_training():
    ...
    assert gpt_train.config_hash(cfg1) == gpt_train.config_hash(cfg2)
...
def test_load_config():
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        ...
        f.flush()
        cfg = gpt_train.load_config(f.name)
...
def test_save_checkpoint_creates_base():
    with tempfile.TemporaryDirectory() as tmpdir:
        cfg = {"n_layer": 1, "n_head": 2, "head_dim": 16, "seq_length": 32}
        model = gpt_train.GPTMini(cfg, vocab_size=20)
        h = gpt_train.config_hash(SMALL_CONFIG)
        gpt_train.save_checkpoint(10, 0.5, SMALL_CONFIG, h, model, tmpdir)
        base = Path(tmpdir) / h
...
def test_save_checkpoint_tier_2():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        model = gpt_train.GPTMini(cfg, vocab_size=20)
        h = gpt_train.config_hash(SMALL_CONFIG)
        gpt_train.save_checkpoint(100, 0.3, SMALL_CONFIG, h, model, tmpdir)
        base = Path(tmpdir) / h
        assert (base / "1" / "model.pth").exists()
...
def test_save_checkpoint_tier_3():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        model = gpt_train.GPTMini(cfg, vocab_size=20)
        h = gpt_train.config_hash(SMALL_CONFIG)
        gpt_train.save_checkpoint(1000, 0.2, SMALL_CONFIG, h, model, tmpdir)
        base = Path(tmpdir) / h
        assert (base / "1" / "model.pth").exists()
        assert (base / "2" / "model.pth").exists()
...
def test_save_checkpoint_config_once():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        gpt_train.save_checkpoint(20, 0.4, SMALL_CONFIG, h, model, tmpdir)
...
def test_save_checkpoint_log_content():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        model = gpt_train.GPTMini(cfg, vocab_size=20)
        h = gpt_train.config_hash(SMALL_CONFIG)
        gpt_train.save_checkpoint(42, 0.123456, SMALL_CONFIG, h, model, tmpdir)
        base = Path(tmpdir) / h
...
def test_save_checkpoint_overwrites():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        h = gpt_train.config_hash(SMALL_CONFIG)
        gpt_train.save_checkpoint(10, 0.5, SMALL_CONFIG, h, model, tmpdir)
        gpt_train.save_checkpoint(20, 0.3, SMALL_CONFIG, h, model, tmpdir)
        base = Path(tmpdir) / h
        meta = json.loads((base / "resume.json").read_text())
...
def test_checkpoint_slot_alternation():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        h = gpt_train.config_hash(SMALL_CONFIG)
        # Even epoch -> slot 0
        gpt_train.save_checkpoint(10, 0.5, SMALL_CONFIG, h, model, tmpdir, optimizer=opt)
        base = Path(tmpdir) / h
        assert (base / "model.0.pth").exists()
        assert (base / "optimizer.0.pt").exists()
        assert not (base / "model.1.pth").exists()
        # Odd epoch -> slot 1
        gpt_train.save_checkpoint(11, 0.4, SMALL_CONFIG, h, model, tmpdir, optimizer=opt)
        ...
        gpt_train.save_checkpoint(12, 0.3, SMALL_CONFIG, h, model, tmpdir, optimizer=opt)
        assert (base / "model.0.pth").exists()
...
def test_find_latest_checkpoint_none():
    assert gpt_train.find_latest_checkpoint("/nonexistent", "abc123") is None
...
def test_find_latest_checkpoint_base_only():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        model = gpt_train.GPTMini(cfg, vocab_size=20)
        h = gpt_train.config_hash(SMALL_CONFIG)
        gpt_train.save_checkpoint(50, 0.3, SMALL_CONFIG, h, model, tmpdir)
        result = gpt_train.find_latest_checkpoint(tmpdir, h)
...
def test_find_latest_checkpoint_tier_newer():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        model = gpt_train.GPTMini(cfg, vocab_size=20)
        h = gpt_train.config_hash(SMALL_CONFIG)
        gpt_train.save_checkpoint(100, 0.2, SMALL_CONFIG, h, model, tmpdir)
        result = gpt_train.find_latest_checkpoint(tmpdir, h)
        assert result is not None
        ep, info, d = result
        assert ep == 100
...
def test_find_latest_checkpoint_wrong_hash():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        h = gpt_train.config_hash(SMALL_CONFIG)
        gpt_train.save_checkpoint(10, 0.5, SMALL_CONFIG, h, model, tmpdir)
        result = gpt_train.find_latest_checkpoint(tmpdir, "wrong_hash")
...
def test_checkpoint_resume_weights():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert result is not None
        ep, info, d = result
        loaded = gpt_train.GPTMini(cfg, vocab_size=20)
...
def test_gpt_mini_min_seq_length():
    ...
    model = gpt_train.GPTMini(cfg, vocab_size=10)
...
def test_generate_text_long_prompt():
    ...
    model = gpt_train.GPTMini(cfg, tok.vocab_size)
    model.eval()
    text = gpt_train.generate_text(model, tok, "the cat sat on the mat the cat sat on the mat",
...
def test_dataset_sequence_boundary():
    ...
    with tempfile.NamedTemporaryFile(suffix=".npy", delete=False) as f:
        tmp_cache = f.name
    try:
        ds = gpt_train.WordDataset(["a b c d e f g h i j"], tok, seq_length=5, cache_file=tmp_cache)
        assert len(ds) >= 1
        last = ds[len(ds) - 1]
...
def test_checkpoint_multiple_models():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        m1 = gpt_train.GPTMini(cfg1, vocab_size=20)
        m2 = gpt_train.GPTMini(cfg2, vocab_size=30)
        h1 = gpt_train.config_hash(config1)
        h2 = gpt_train.config_hash(config2)
        gpt_train.save_checkpoint(10, 0.5, config1, h1, m1, tmpdir)
        gpt_train.save_checkpoint(20, 0.3, config2, h2, m2, tmpdir)
        r1 = gpt_train.find_latest_checkpoint(tmpdir, h1)
        r2 = gpt_train.find_latest_checkpoint(tmpdir, h2)
...
def test_vocab_hash_stable():
    h1 = gpt_train.get_vocab_hash({"max_vocab_size": 32768}, ["/tmp"])
    h2 = gpt_train.get_vocab_hash({"max_vocab_size": 32768}, ["/tmp"])
    assert h1 == h2
...
def test_vocab_hash_changes_on_vocab_size():
    h1 = gpt_train.get_vocab_hash({"max_vocab_size": 32768}, ["/tmp"])
    h2 = gpt_train.get_vocab_hash({"max_vocab_size": 16384}, ["/tmp"])
    assert h1 != h2
...
def test_vocab_conf_hash_stable():
    ...
    h1 = gpt_train.get_vocab_conf_hash(cfg, sources)
    h2 = gpt_train.get_vocab_conf_hash(cfg, sources)
...
def test_vocab_conf_hash_changes_on_sources():
    cfg = {"max_vocab_size": 32768, "sentence_sample_cap": 25000000}
    h1 = gpt_train.get_vocab_conf_hash(cfg, ["tinystories"])
    h2 = gpt_train.get_vocab_conf_hash(cfg, ["tinystories", "wikipedia_en_corpus"])
...
def test_corpus_conf_hash_stable():
    ...
    h1 = gpt_train.get_corpus_conf_hash(sources)
    h2 = gpt_train.get_corpus_conf_hash(sources)
...
def test_corpus_conf_hash_changes_on_sources():
    h1 = gpt_train.get_corpus_conf_hash(["tinystories"])
    h2 = gpt_train.get_corpus_conf_hash(["tinystories", "wikipedia_en_corpus"])
...
def test_try_conf_cache_miss():
    with tempfile.TemporaryDirectory() as tmpdir:
        cd = Path(tmpdir)
        result = gpt_train._try_conf_cache(cd, "a1b2c3d4e5f61234", "1234567890abcdef")
...
def test_write_read_cache_lock():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ckpt.mkdir()
        gpt_train.write_cache_lock(str(ckpt), "vocab-x.json", "data-y.npy")
        lock = gpt_train.read_cache_lock(str(ckpt))
...
def test_read_cache_lock_missing():
    with tempfile.TemporaryDirectory() as tmpdir:
        lock = gpt_train.read_cache_lock(tmpdir)
        assert lock is None
...
def test_save_checkpoint_writes_cache_lock():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        model = gpt_train.GPTMini({"n_layer": 1, "n_head": 2, "head_dim": 8, "seq_length": 8}, 50)
