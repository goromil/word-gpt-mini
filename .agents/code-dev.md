...
1. Define class in `gpt_mini3.py` model section
2. Wire into `GPTMini.__init__()` and `forward()`
3. Verify parameter count with `calc_params.py`
4. Add test to `test_gpt_mini3.py`

#### Changing config schema
1. Update `gpt_mini3.json` with new key
2. Update `gpt_mini3_draft.json` for designer defaults
...
1. Add prefix to `tokenizer.sources` and/or `training.sources` in `gpt_mini3.json`
...
wsl /home/george/miniconda3/envs/ai/bin/python /mnt/c/Users/gorom/source/ai/word-gpt-mini/test_gpt_mini3.py
