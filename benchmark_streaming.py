...
def bench_vocab_hash():
    ...
    # Warm-up
    h = gpt_mini3.get_vocab_hash(vocab_cfg, data_dirs)
    ...
    for _ in range(N):
        t0 = time.perf_counter()
        h2 = gpt_mini3.get_vocab_hash(vocab_cfg, data_dirs)
...
def bench_corpus_hash():
    ...
    # Warm-up
    h = gpt_mini3.compute_corpus_hash(data_dirs)
    ...
    for _ in range(N):
        t0 = time.perf_counter()
        h2 = gpt_mini3.compute_corpus_hash(data_dirs)
        times.append(time.perf_counter() - t0)
        assert h == h2, "Hash not stable!"
...
def bench_iterator_count():
    ...
    iter_ = gpt_mini3.SentenceIterator(data_dir, extra_dirs, sources=sources)
...
def bench_iterator_memory():
    ...
    tracemalloc.start()
    iter_ = gpt_mini3.SentenceIterator(data_dir, extra_dirs)
...
def bench_end_to_end():
    ...
    # Phase 1: Hashes
    t0 = time.perf_counter()
    vh = gpt_mini3.get_vocab_hash(vocab_cfg, data_dirs)
    ch = gpt_mini3.compute_corpus_hash(data_dirs)
    hash_time = time.perf_counter() - t0

    # Phase 2: Iterator
    t0 = time.perf_counter()
    sources = vocab_cfg.get("sources")
    si = gpt_mini3.SentenceIterator(paths["data_dir"], paths.get("extra_data_dirs"),
                                     sources=sources)
