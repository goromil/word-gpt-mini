...
```bash
python train_designer.py --no-interact --force   # generate train_gpt.json
...
- `train_gpt_draft.json` — draft architecture config (model, vocab, training defaults, paths)
- `train_gpt.json` — working config generated by `train_designer.py` (model, training, tokenizer, paths)
...
python calc_params.py train_gpt.json            # custom config path
```
...
## Draft Config Format (`train_gpt_draft.json`)
...
## Working Config Format (`train_gpt.json`)
