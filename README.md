...
```bash
python train_designer.py --no-interact --force   # generate gpt_train.json
python noipc_ddp_train.py -d 0,1                 # train on 2 RTX 3090s (no-P2P)
python ipc_ddp_train.py -d 0,1                   # train on 2 V100 SXM2 (P2P/NVLink)
```

## Config Files

- `gpt_train_draft.json` — draft architecture config (model, vocab, training defaults, paths)
- `gpt_train.json` — working config generated by `train_designer.py` (model, training, tokenizer, paths)
...
python calc_params.py gpt_train.json            # custom config path
```
...
|---|---|---|---|
| `ipc_ddp_train.py` | DDP GPU all_reduce | P2P (NVLink, SXM2) | V100, A100, H100 |
| `noipc_ddp_train.py` | CPU all_reduce (gloo) | No P2P needed | RTX 3090, consumer GPUs |
...
```bash
# P2P GPUs (NVLink):
python ipc_ddp_train.py -d 0,1               # train on 2 GPUs
python ipc_ddp_train.py -d 0,1,2,3           # train on 4 GPUs

# No-P2P GPUs (PCIe bridge):
python noipc_ddp_train.py -d 0,1             # train on 2 GPUs, CPU grad sync
```
...
| File | Purpose |
|---|---|
| `services/start.sh` | Bash script: activates conda `ai` env, runs `noipc_ddp_train.py`, logs to `E:\training\logs\` |
